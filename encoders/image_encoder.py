...
class ImageEncoder:
    ...
    def encode_images_from_directory(
            self, dir_path: str, output_file: str, invert_colors: bool = False, size: tuple = (7, 7)
    ):
        """
        ...
        for sub_dir_name in sorted(os.listdir(dir_path)):
            ...
            for file_name in sorted(os.listdir(sub_dir_path)):
                ...
                if os.path.isfile(file_path):
                    try:
                        layer = self.encode_image_to_array(file_path, invert_colors, size)
