...
from tools.support_functions import ActivationFunctions, NormalizationFunctions, InitializationFunctions


class GeneralTestParameters(unittest.TestCase, ActivationFunctions, NormalizationFunctions, InitializationFunctions):
...
class TestConfigurationMethods(GeneralTestParameters):
    ...
    def test_get_json_data_file_not_found(self, mock_file):
        with self.assertRaises(FileNotFoundError) as context:
            get_json_data('configuration', 'non_existing_file')
        self.assertEqual(str(context.exception), 'Файл не найден: configuration/non_existing_file.json')
        mock_file.assert_called_once_with('configuration/non_existing_file.json', encoding='UTF-8')
...
class TestNormalizationFunctions(GeneralTestParameters):
    def test_get_softmax_list(self):
        result = self.get_softmax([1.0, 2.0, 3.0])
        expected = [0.0900, 0.2447, 0.6652]
        for r, e in zip(result, expected):
            self.assertAlmostEqual(r, e, places=4, msg="Ошибка вычисления get_softmax")
